...
SELECT * FROM grupos ORDER BY grupo, codigo_time
SELECT * FROM times
...
BEGIN
    ...
    BEGIN
        ...
        DECLARE @id_mandante INT, @id_visitante INT, @qtde_times INT, @qtde_jogos INT;
        SELECT @qtde_times = COUNT(*) FROM @times;
        SET @qtde_jogos = @qtde_times / 2;
        
        WHILE @qtde_jogos > 0
        BEGIN
            SELECT TOP 1 @id_mandante = id, @id_visitante = (SELECT TOP 1 id FROM @times WHERE id != @id_mandante AND nome NOT IN (SELECT visitante FROM @jogos WHERE data_jogo = @data)) 
            FROM @times WHERE nome NOT IN (SELECT mandante FROM @jogos WHERE data_jogo = @data) ORDER BY NEWID();
            
            INSERT INTO @jogos (mandante, visitante, data_jogo)
            SELECT nome, (SELECT nome FROM @times WHERE id = @id_visitante), @data FROM @times WHERE id = @id_mandante;
            
            DELETE FROM @times WHERE id IN (@id_mandante, @id_visitante);
            SET @qtde_jogos = @qtde_jogos - 1;
        END
        
        SELECT * FROM @jogos WHERE data_jogo = @data;
        DELETE FROM @jogos WHERE data_jogo = @data;
        
        IF @rodada IN (1, 7) SET @data = '2023-05-03';
        ELSE IF @rodada IN (2, 8) SET @data = '2023-05-07';
        ELSE IF @rodada IN (3, 9) SET @data = '2023-05-10';
        ELSE IF @rodada IN (4, 10) SET @data = '2023-05-14';
        ELSE IF @rodada IN (5, 11) SET @data = '2023-05-17';
        ELSE IF @rodada IN (6, 12) SET @data = '2023-05-21';
